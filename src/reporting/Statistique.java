...
public class Statistique extends JFrame {
    ...
    public void RequeteStat() throws SQLException {
        ...
        for (int i = 0; i < nb_service; i++) {
            ResultSet resultat5 = connect.getStmt().executeQuery("SELECT SUM(salaire) AS total FROM infirmier WHERE code_service = '"
                    + tab_service_code[i] + "'");
            resultat5.next();
            //JLabel a = new JLabel(tab_service_nom[i]+" : "
            //+resultat5.getInt("total") + "€"); //Nom service
            dataset.addValue(resultat5.getInt("total"), tab_service_nom[i],
                    new Integer(i));
            //salaire.add(a);
        }
        //droite.add(salaire);
